...
type NaiveOrderBook private (lastOrderId : OrderId, bids : List<OrderBookPricePoint>, asks : List<OrderBookPricePoint>, executionEvent : Event<OrderExecution * IOrderBook>) =
    ...
    new () = NaiveOrderBook(OrderId.Zero, [], [], new Event<OrderExecution * IOrderBook>())
        
    member private b.Bids = bids
    member private b.Asks = asks

    member private b.IncrementLastOrderId () = let nextOrderId = OrderId.Increment lastOrderId in (nextOrderId, new NaiveOrderBook(nextOrderId, b.Bids, b.Asks, executionEvent))
    ...
    member private b.AddOrder order =
        ...
        let nob = match order.Side with
                  | Buy -> new NaiveOrderBook(newOrderId, newPps, b.Asks, executionEvent)
                  | Sell -> new NaiveOrderBook(newOrderId, b.Bids, newPps, executionEvent)
                                          ...
                                          | e::es -> executeTrade(order, e, pp.Price)
                                                  |> (fun (oRem, eRem, x) -> match eRem.Size > OrderSize.Zero with
                                                                             ...
                                                                             | false -> match es with
                                                                                        ...
                                                                                        | [] -> (oRem, pps, x))
        let nob = match order.Side with
                  | Buy -> new NaiveOrderBook(lastOrderId, b.Bids, pps, executionEvent)
                  | Sell -> new NaiveOrderBook(lastOrderId, pps, b.Asks, executionEvent)
    ...
    member private b.CancelOrder orderId =
        ...
        | (pps, true) -> (true, new NaiveOrderBook(lastOrderId, pps, b.Asks, executionEvent))
        | (_, false) -> match removeFromPs asks with
                        | (pps, true) -> (true, new NaiveOrderBook(lastOrderId, b.Bids, pps, executionEvent))
                        | (_, false) -> (false, b)
